...
**The core insight: SSR is a single, synchronous, one-pass evaluation.** No re-renders, no effects, setters are never called. So the server doesn't need a React runtime — only the component's "render slice," whose semantics are small enough to compile straight to Go. The result: a 50-item list page renders in **~15 µs** of Go (straight-line writes, ~140 allocations); a 4-vCPU GitHub runner serves it at **~20k req/s** — the same as templ, ~4× `html/template`, ~65× Next.js — at **21 MB RSS** with a **20 ms cold start**; first-load JS is **9 KB gzipped** (signals runtime + loader + islands; the morphing library loads on first navigation), `go build` produces a single binary, and `delve` / `pprof` / `go test` all just work. Numbers and method: [`bench/`](bench/README.md).
...
| `client/` | signals, `el/t/text/cond/each` (keyed reuse), resumable hydration, island loader, SPA nav, progress bar, prefetch, stores (`createStore`, copy-on-write `set`, seeds re-applied on navigation), cross-island `emit`/`on`, i18n, dev live reload |
...
- **Stores**: `export const cart = createStore<CartView>({ items: [], empty: true })` in a client module is state shared by islands — every field a signal (`const { count } = cart`), `cart.set((s) => { s.items.push(x); })` or `cart.set(await add(id))` from handlers with copy-on-write semantics (untouched rows keep identity, quiet fields stay quiet), and `seed(cart, view(sid))` in the page or layout body so the first paint already shows the server's value and the browser hydrates from it — no flash, no refetch, no event bus for state. The shop's cart badge, cart page and checkout share one store.
