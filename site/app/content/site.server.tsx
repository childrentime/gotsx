...
export function syntaxRows(locale: string): Row[] {
  ...
  return [
    ...
    { cat: c("module"), syntax: "import { createStore, seed } from \"gotsx\"", note: loc(locale, "stores: state shared by islands, seeded per request by the page (see the Hooks rows)", "store: 多个岛共享的状态, 由页面每请求注入初值(见 Hooks 行)"), ok: true },
    ...
    { cat: c("hooks"), syntax: "export const cart = createStore<T>(init)", note: loc(locale, "module-level const of a client module (app/stores/cart.client.tsx): a store — one signal per field in the browser, the request's seeded value on the server; fields missing from init are zero values; state is JSON (no functions, no Node)", "client 模块的模块级 const(app/stores/cart.client.tsx): 一个 store —— 浏览器里每个字段一个 signal, 服务端是本请求注入的值; init 里缺的字段是零值; 状态必须能 JSON 化(没有函数和 Node)"), ok: true },
    { cat: c("hooks"), syntax: "cart.count / const { count, items } = cart", note: loc(locale, "reads are signals (fine-grained bindings, memos, effects); client modules only; read-only outside set", "读取就是 signal(细粒度绑定、memo、effect); 只在 client 模块; set 之外只读"), ok: true },
    { cat: c("hooks"), syntax: "cart.set((s) => { s.items.push(x); s.count += 1; }) / cart.set(value)", note: loc(locale, "from handlers, effects and plain functions, never during render: s is a copy-on-write draft — only the fields that changed notify, untouched rows of a keyed list keep their identity; a whole value replaces the state", "在事件处理、effect 和普通函数里调用, 不能在渲染中: s 是写时复制的 draft —— 只有改过的字段通知订阅者, keyed 列表里没动的行保持身份; 传整个值则整体替换"), ok: true },
    { cat: c("hooks"), syntax: "seed(cart, value)", note: loc(locale, "in the body of a page or layout's default component, before the JSX: the islands of this request render with value and the browser's store starts from it (no flash, no refetch)", "在页面或布局默认组件的函数体里、JSX 之前: 本请求的所有岛用这个值渲染, 浏览器里的 store 也从它开始(首屏不闪, 不用再请求)"), ok: true },
