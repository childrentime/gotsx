...
export default function Home({ query, locale, path }: PageProps) {
  ...
  return (
    <Layout title={loc(lc, "A Go-native full-stack framework in a TSX dialect", "TSX 方言, Go 原生的全栈框架")} locale={lc} path={path}>
      {/* hero */}
      <section class="container-page fade-up pb-16 pt-20">
        <div class="mb-5 flex flex-wrap items-center gap-2">
          <Badge color="brand">v0.6</Badge>
          <Badge>{loc(lc, "No Node", "零 Node")}</Badge>
          <Badge>{loc(lc, "No JS engine", "零 JS 引擎")}</Badge>
          <Badge>{loc(lc, "No npm", "零 npm")}</Badge>
        </div>
        <h1 class="max-w-3xl text-4xl font-semibold leading-[1.1] tracking-tight md:text-6xl">
          {loc(lc, "React's ideas,", "借 React 的思想,")}<br />
          {loc(lc, "compiled to ", "编译到 ")}<span class="font-mono">Go</span>{loc(lc, "", " 的全栈框架")}
        </h1>
        ...
        <div class="mt-8 flex flex-wrap items-center gap-3">
          ...
          <span class="ml-2 text-sm text-muted-foreground">{loc(lc, "← these buttons are a component library written in the dialect", "← 这些按钮是方言写的组件库")}</span>
        </div>
        <div class="mt-10 flex flex-wrap items-center gap-4">
          <Counter start={start} />
          <span class="max-w-md text-sm leading-6 text-muted-foreground">{loc(lc, "This button is an island: Go rendered it, and after the browser hydrates it, it's a signal. Click it.", "这个按钮是一个岛: Go 渲染了它, 浏览器 hydrate 后它是一个 signal。点它。")}</span>
        </div>
      </section>
      ...
      <section class="container-page prose-code pt-20">
        <h2 class="text-2xl font-semibold tracking-tight">{loc(lc, "One source, two compilers", "一份源码, 两个编译器")}</h2>
        <p class="mt-2 max-w-2xl leading-7 text-muted-foreground">
        ...
        <div class="mt-6 grid gap-6 lg:grid-cols-2">
          <div class="min-w-0">
            <div class="mb-2 text-sm font-medium">{loc(lc, "Server page → Go", "服务端页面 → Go")}</div>
          ...
          <div class="min-w-0">
            <div class="mb-2 text-sm font-medium">{loc(lc, "Island → Go + JS", "岛 → Go + JS")}</div>
      ...
      <section class="container-page pt-16">
        <h2 class="text-2xl font-semibold tracking-tight">{loc(lc, "Features", "特性")}</h2>
      ...
      {/* honesty */}
      <section class="container-page pt-16">
        <h2 class="text-2xl font-semibold tracking-tight">{loc(lc, "What it is not", "它不是什么")}</h2>
        <div class="mt-6 grid gap-4 md:grid-cols-3">
          ...
          <Card title={loc(lc, "Not Next", "不是 Next")}>
            {loc(lc, "No RSC protocol, no server actions, no client-side data fetching framework. Streaming is a <Suspense> boundary resolved in a goroutine; what you get is Go's speed, a single binary, and a 6KB client.", "没有 RSC 协议、没有 server actions、没有客户端数据获取框架。流式渲染是一个在 goroutine 里求值的 <Suspense> 边界; 有的是 Go 的速度、单二进制和 6KB 客户端。")}
          </Card>
        </div>
      </section>
      ...
      <section class="mx-auto max-w-3xl px-6 pt-16">
        <h2 class="text-2xl font-semibold tracking-tight">{loc(lc, "FAQ", "常见问题")}</h2>
        <div class="mt-6"><Accordion items={faqs(lc)} /></div>
      </section>

      <section class="container-page pt-20 text-center">
        <h2 class="text-2xl font-semibold tracking-tight">{loc(lc, "Up and running in five minutes", "五分钟跑起来")}</h2>
