// gomu: a full-stack e-commerce demo. Pages = the dialect compiled to Go; writes = plain Go actions; sessions = a sid cookie.
package main
...
func main() {
	...
	dev := flag.Bool("dev", false, "development mode (error pages with stacks, assets no-cache)")
	flag.Parse()
